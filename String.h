...
namespace str {
    ...
    short Countstring(string text, enWhattoCount what = enWhattoCount::Allletters)
    {
        int count = 0;
    ...
    string ReplaceString(string text, string replace_From, string replace_To) 
    {
        int pos = text.find(replace_From);

        while (pos != string::npos)
        {
            text.replace(pos, replace_From.length(), replace_To);

            pos = text.find(replace_From);
        }
